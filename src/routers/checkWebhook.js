...
async function checkWebhook(req, res) {
    try {
        const dbFields = await User.findFieldsByPortal(globalThis.companyDomain)
        ...
        if (currentEDRPO != previousEDRPO) {
            ...
            if (String(currentEDRPO).length == 8) {
                ...
                try { //41750253
                    ...
                    const result = await api.getTOV(currentEDRPO, globalThis.companyDomain)
                ...
                try { //3461610104
                    ...
                    const result = await api.getFOP(currentEDRPO, globalThis.companyDomain)
